...
for (let i = 1; i < lines.length; i++) {
  ...
  for (let j = 0; j < col_names.length; j++) {
    obj[col_names[j].replace('\r','')] = cols[j];
  }
  let obj_x = obj.x;
  let obj_y = obj.y;
  if (obj_x <= 0) {
...
for (let grid_id in grids_logs) {
  ...
  // 추후 클러스터 갯수대로 변환
  let size_means = { '0': [], '1': [] };
  // console.log(JSON.stringify(grid_logs));
  for (let i = 0; i < grid_logs.length - 1; i++) {
    // console.log(grid_logs[i].cluster_id);
    size_means[grid_logs[i].cluster_id.trim()].push(grid_logs[i].size);
  }
...
for (let i = 0; i < adaptive_grid_cells2.length; i++) {
  fs.appendFileSync(`./clustering_result2/4801_area_s_big.txt`, `${JSON.stringify(adaptive_grid_cells2[i])},\n`);
}
for (let i = 0; i < adaptive_grid_cells1.length; i++) {
  fs.appendFileSync(`./clustering_result2/4801_area_s_small.txt`, `${JSON.stringify(adaptive_grid_cells1[i])},\n`);
}
