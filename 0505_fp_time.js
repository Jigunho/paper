...
for (let target_id in targets_logs) {
  ...
  for (let i = 0; i < target_logs.length; i++) {
    ...
    if (r == -1) {
      ...
      user_adaptive_grids2.push(g_size.id);
  ...
  console.log(`${target_id} - ${JSON.stringify([...new Set(user_adaptive_grids2)])}`)
...
// console.log(upper_transactions[0])
let merged_t = [upper_transactions[0]];
for (let i = 1; i < upper_transactions.length; i++) {
  let init_len = merged_t.length;
  let cnt = 0 ;
  for (let m = 0 ; m < init_len; m ++) {
    // console.log(init_len)
    let min_len = mathjs.min(merged_t[m].length, upper_transactions[i].length);
    let sum_arr = merged_t[m].concat(upper_transactions[i]);
    let dup_removed = [...new Set(sum_arr)];
    if (min_len / dup_removed.length > 0.7) {
      merged_t[m] = dup_removed;
      // console.log(`${i}, merged`)
      break;
    } else {
      // console.log('ww')
      // console.log(`${i}, ${m}`)
      // merged_t.push(upper_transactions[i]);
      // break;
